...
$this->context['readonly']=TRUE;
        ...
        $inputView = new Projektor2_View_HTML_Element_Input();
        $fieldsetElement->addChild($inputView
                ->assign('label', 'Jméno: ')
                ->assign('attributes', array('id'=>'dotaznik->jmeno', 'type'=>'text', 'name'=>'dotaznik->jmeno', 'size'=>'20', 'maxlength'=>'30', 'value'=>$this->context['dotaznik->jmeno']))
                ->assign('readonly', $readonly));
        $inputView = new Projektor2_View_HTML_Element_Input();
        $fieldsetElement->addChild($inputView
                ->assign('label', 'Příjmení: ')
                ->assign('attributes', array('id'=>'dotaznik->prijmeni', 'type'=>'text', 'name'=>'dotaznik->prijmeni', 'size'=>'20', 'maxlength'=>'30', 'value'=>$this->context['dotaznik->prijmeni']))
                ->assign('readonly', $readonly));

        $inputView = new Projektor2_View_HTML_Element_Input();
        $fieldsetElement->addChild($inputView
                ->assign('attributes', array('type'=>'submit', 'name'=>'B1', 'value'=>'Uložit'))
                ->assign('readonly', $readonly));
        $inputView = new Projektor2_View_HTML_Element_Input();
        $fieldsetElement->addChild($inputView
                ->assign('attributes', array('type'=>'reset', 'name'=>'B2', 'value'=>'Zruš provedené změny'))
                ->assign('readonly', $readonly));
        $inputView = new Projektor2_View_HTML_Element_Input();
        if ($this->context['dotaznik->id_zajemce']){
            $inputView = new Projektor2_View_HTML_Element_Input();
            $fieldsetElement->addChild($inputView
                    ->assign('attributes', array('type'=>'submit', 'name'=>'pdf', 'value'=>'Tiskni dotazník'))
                    ->assign('readonly', $readonly));
        }
