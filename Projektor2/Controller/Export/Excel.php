    ...
    public function getResult() {
        if($this->request->post('dbtabulka') AND substr($this->request->post('dbtabulka'),0,3)<>"---") {
            ...
            if (Projektor2_Model_File_ExcelMapper::save($modelExcel, $this->sessionStatus)) {
                ...
                $parts[] = new Projektor2_View_HTML_ExportExcel();
                }
        } else {
            switch ($this->sessionStatus->projekt->kod) {
                case 'HELP':
                    $parts[] = new Projektor2_View_HTML_Help_Export();
                    break;
                case 'AP':
                    $parts[] = new Projektor2_View_HTML_Ap_Export();
                    break;
                case 'SJZP':
                    $parts[] = new Projektor2_View_HTML_Sjzp_Export();
                    break;
                default:
                    break;
            }
